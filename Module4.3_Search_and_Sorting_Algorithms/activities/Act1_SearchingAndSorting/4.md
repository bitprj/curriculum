...
This is the code used to test our DFS algorithm. We can use this block to check if our function works. Feel free to play around with it. 
...
Now, you are might be wondering, why does this function call itself? DFS is supposed to have a stack, but I don't see any such data structure? To answer these valid concerns, we must diverge from the code and discuss how arguments are passed to a function.

Deep down in one's computer, arguments to a function are passed through a stack! 

![](https://images.pexels.com/photos/2280545/pexels-photo-2280545.jpeg?cs=srgb&dl=stack-of-pancakes-with-strawberry-fruits-2280545.jpg&fm=jpg)

I wish we were talking about a stack of pancakes, but we are actually talking about the stack with the **L**ast-**I**n **F**irst-**O**ut (**LIFO**) functionality! When a function is called, each argument is pushed on to a stack. When a function terminates, all of the arguments are popped from the stack. 
...
The diagram below explains DFS step-by-step with special attention given to how recursion, passing arguments, and the stack play a role. Please read it carefully!
