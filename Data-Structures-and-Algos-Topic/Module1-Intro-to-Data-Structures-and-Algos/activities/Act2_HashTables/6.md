...
An example of linear probing can be explained with phone numbers. Lets say that we have a list of phone numbers and we're trying to assign them correctly to each person based on their area code, but we don't want a collision such that one person can have the exact same phone number. This is where linear probing comes into play. The image below visually gives you the concept behind this main idea.
