...
3. `depth` is an integer intially set to 0. It indicates the number of guessing and determines if it's allowed to try guessing. If it `depth` is less than 2 (`depth < 2)`, then guessing a move is allowed. 
...
- 1) An easy iteration through all spots: within each number we will check by row, column and box.
- 2) Guessing clause which will be the fallback guessing and checking mechinism (this segment uses a function called `testPossible()` which we have not yet defined)
