...
To illustrate how this code works, let's use the tree from the first card as as an example. We will add a node `11` to this tree. Below, you can see the specific path that `BSTSearch` takes to insert the node as well as the actual position that `11` ends up in:
